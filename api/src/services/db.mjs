  ...
  if (parseInt(questCount?.cnt || '0', 10) === 0) {
    await p.query(`
      INSERT INTO quests (slug, title, description, quest_type, seeds_reward, icon, repeatable, sort_order) VALUES
        ('follow-x',       'Follow @growstreams on X',             'Follow the official GrowStreams account on X (Twitter).',          'X_FOLLOW',        100, 'twitter',    FALSE, 1),
        ('mention-x',      'Post on X mentioning @growstreams',    'Create a post on X that mentions @growstreams.',                   'X_MENTION',       150, 'megaphone',  TRUE,  2),
        ('star-repo',      'Star the GrowStreams repo',            'Star the GrowStreams repository on GitHub.',                       'GITHUB_STAR',     100, 'star',       FALSE, 3),
        ('raise-pr',       'Raise a PR on GrowStreams repo',       'Open a pull request on the GrowStreams GitHub repository.',        'GITHUB_PR',       200, 'git-pull-request', TRUE, 4),
