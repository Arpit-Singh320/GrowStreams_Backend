...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${BACKEND_API}/api/analytics/history${path}`, {
      cache: 'no-store',
      headers: {
    ...
    console.error('Analytics history proxy error:', error)
    return NextResponse.json(
      { error: 'Failed to fetch history data', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
